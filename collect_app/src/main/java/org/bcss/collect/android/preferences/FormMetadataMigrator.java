...
import static org.bcss.collect.android.preferences.PreferenceKeys.KEY_METADATA_MIGRATED;
...
public class FormMetadataMigrator {
    ...
    /** The migration flow, siteName source to target */
        ...
        if (! migrationAlreadyDone) {
            ...
            for (String[] pair : SOURCE_TARGET_VALUE_PAIRS) {
                ...
                if (! migratingValue.isEmpty()) {
                    Timber.i("Copying %s siteName %s to %s", migratingValue, pair[0], pair[1]);
